...
- CSN (Circuit Switching Network)
	...
	- 최선의 경로를 미리 설정해두고, 패킷을 보내기 전에 중앙 제어 방식으로 경로를 전달한다.
- PSN (Packet Switching Network)
	- 경로 설정 시 중앙 제어 없이 목적지 주소 전달 방식
	- 택배와 유사하게 동작한다.
		- 창고에서 창고로, 맞는 길을 따라 라우팅을 해가며 찾아간다.
	- 최선의 경로를 알고 있지 않으며, 현재 가는 목적지의 주소만 생각하고 라우트를 따라간다.
- 인터넷은 PSN 기반의 네트워크이며, 따라올 수 밖에 없는 특징들이 있다.

## OSI Layers
- 각 레이어에서 하는 일을 모듈화할 수 있다.
	- 통신이 일어나는 과정을 단계별로 파악할 수 있다.
- 계층을 나누면, 같은 레이어끼리 통신을 할 수 있게 된다.
	- 필요한 일은 카테고리 별로 모듈화한다.

### Application
- 7계층에서 Application, Presentation, Session을 합쳐 Application으로 볼 것이다.
	- TCP/IP 5계층과 OSI 7계층의 차이이다.
	- 인터넷의 개념적 레이어는 원래 다섯개이다.

### Transport
- 데이터의 전송을 담당한다.
- process에서 process로 message를 딜리버리한다.
	- 패킷을 보낼때 어떤 어플리케이션으로 보내는지 구분하기 위한 레이어다.
		- 하나의 컴퓨터에서 여러가지 어플리케이션이 동시에 네트워크를 사용하면, 정확히 어떤 어플리케이션으로 오는 트래픽인지 구분할 수 있어야 한다.
		- 프로세스를 구분하기 위해서 port number를 할당한다.

### Network
- source to destination으로 packet을 이동시킨다. 
	- 인터넷은 PSN을 이용하고, 따라서 최종 목적지에 대한 값만 갖고 있따.
	- 이때 사용하는 것이 논리적 주소인 IP 주소이다.
		- IP주소는 Unique하여, 패킷을 원하는 목적지까지 전송할 수 있다.
- 패킷을 목적지까지 전송하는 것이다.
	- 패킷이 전송되는 경로를 관리하는 계층이다.
- Router가 대표적인 장비이다.
	- 각 라우터는 다음 목적지로 가려면 어디로 가야할지에 대한 정보를 라우팅 테이블에 저장한다.

### Data Link
- 데이터의 물리적인 hop-to-hop delivery를 수행하게 된다.
	- src to dest를 수행하기 위해서는 단계별로 이동해야하고, 주소가 중간 목적지에 도착할 때 마다 달라진다.
	- 중간 목적지에서 다음 위치로 이동할 수 있도록 하고, 통신에서의 오류도 찾는 기능을 한다.
		- 안전한 정보 전달을 수행할 수 있도록 돕는다.
- frame 단위로 비트를 구성한다.
- 학기중 다루진 않지만, 이해하고 있어야 한다.
- MAC 주소를 이용해 데이터를 해당 장비로 보내고, LLC는 각 장비를 논리적으로 연결하는 역할을 한다.

### Physical
- 수업 중 다루지 않을 것이다.
- 각 비트를 다음 노드까지 보내는 일에 대한 책임을 진다.
...
- A-F까지를 Hop-to-Hop, A-P를 Source-to-Dest라 한다.
	...
	- 라우터는 자신의 MAC 주소를 본인이 알고 있다.
		...
		- 라우터도 라우팅 테이블을 바탕으로 동일하게 탐색을 진행하고, 결과로 보내주게 된다.
			- IP주소를 통해 MAC 주소를 알아내고, 이를 통해 다음 목적지로 향할 수 있게 된다.
...
- 스위치가 MAC 내부에 있는지 알고 있으면 목적지에만 보내고, 모르면 연결된 모든 디바이스로 다 보낸다.
...
- ip 주소는 unique하다.
	- ip 주소는 하나뿐인데, 왜 Hop-to-Hop에서 MAC 주소를 사용할까?
		...
		- 네트워크 레이어의 주소 체계가 다양했다. 인터넷은 IP를 사용하지만 네트워크간의 연결에 다른 프로토콜도 사용했다.
			- 로우 레벨에는 MAC 주소로 통일하면 Network Layer가 다른 프로토콜일지라도 LAN에서는 문제 없이 동작하도록 할 수 있다.
